...
system.time({
  result <- ddtlcm_fit(K = K, data = response_matrix, 
                     item_membership_list = item_membership_list, total_iters = 50)
})

print(result)
```

We next summarize the posterior chain by discarding the first 300 iterations as burn-ins (`burnin = 300`). To deal with identifiability of finite mixture models, we perform post-hoc label switching using the Equivalence Classes Representatives (ECR) method by specifying `relabel = T`. To save space in the document, we do not print the summary result here (`be_quiet = T`).
```{r}
burnin <- 30
summarized_result <- summary(result, burnin, relabel = T, be_quiet = T)
```

We can visualize the summarized result, including the MAP tree and the class profiles:
