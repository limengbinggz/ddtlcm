test_that("'summary.ddt_lcm' returns a valid summary of DDT-LCM", {
  ...
  summarized_result <- summary(result_diet_shortchain, burnin = 50, relabel = TRUE, be_quiet = TRUE)
  K <- result_diet_shortchain$setting$K
  J <- length(unlist(result_diet_shortchain$setting$item_membership_list))
