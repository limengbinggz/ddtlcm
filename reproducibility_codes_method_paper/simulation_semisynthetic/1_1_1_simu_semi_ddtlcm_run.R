...
c_init <- rgamma(1, 5, 1)
# initialize randomly
initialization <- initialize(K, response_matrix, num_items_per_group = J_g, c = c_init,
...
# set.seed(seed)
system.time({
  result <- ddt_lcm_mcmc(K, data = response_matrix, num_items_per_group = J_g,
...
Sigma_dat <- data.table(N = N, seed_theta = seed_theta, seed_Y = seed_Y,
                        var_name = names(bias), 
...
save(seed_theta, seed_Y, sim_data,  
     initials, result, summarized_result_ddtlcm, Sigma_dat, file = file_name)
