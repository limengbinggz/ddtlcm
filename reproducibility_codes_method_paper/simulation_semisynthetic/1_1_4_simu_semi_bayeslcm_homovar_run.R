...
#   initialization <- try(initialize(K, response_matrix, num_items_per_group = J, c = c_init,
...
# note that num_items_per_group = J instead of J_g
system.time({
  result <- ddt_lcm_mcmc_independent(K, data = response_matrix, num_items_per_group = J,
...
save(seed_theta, seed_Y, initials, result, 
     summarized_result_bayeslcm_homovar, file = file_name)
