...
source("functions/MH_independent.R")
total_iters <- 8000
burnin <- 5000
controls <- list()
# set.seed(seed)
system.time({
  result <- ddt_lcm_mcmc_independent(K, data = response_matrix, num_items_per_group,
...
dat_result[row_idx, c("label", "N")] <- c("BayesLCM (heterogeneous var) + HC", N)
...
for (method_hclust in method_hclust_list){
  ...
  if (method_hclust == method_hclust_list[1]){
    ...
    dat_result[row_idx, ] <- c(seed_theta, seed_Y, fix_tree_at, 
                               ...
                               "BayesLCM (heterogeneous var) + HC", N)
  }
  row_idx <- row_idx + 1
}

summarized_result_bayeslcm_heterovar <- dat_result
...
save_dir <- paste0(save_dir, "/bayeslcm_heterovar")
if (!dir.exists(save_dir)){
  print("create")
  dir.create(save_dir)
}
file_name <- paste0(save_dir, "/simdat_seed", seed_Y, ".RData")
save(seed_theta, seed_Y, initials, result, 
     summarized_result_bayeslcm_heterovar, file = file_name)
