...
sample_sigmasq <- function(shape0, rate0, dist_mat, num_items_per_group, locations){
  ...
  for (g in 1:G) {
    ...
    # cat("dim locations_g_diff: ", dim(locations_g_diff),"\n")
    # cat("dim dist_mat[[g]]$Dhalf_Ut: ", dim(dist_mat[[g]]$Dhalf_Ut),"\n")
    # D_half_X <- dist_mat[[g]]$Dhalf_Ut %*% locations_g_diff
  ...
  # cat("Sigma_by_group =", Sigma_by_group, "\n")
  # cat("dist_mat[[g]]$Dhalf_Ut =", dist_mat[[g]]$Dhalf_Ut, "\n")
...
#'@return a numeric vector of G elements, each being the newly sampled variance
#'    of the latent location of this group
sample_leaf_locations_pg <- function(num_items_per_group, root_location, dist_mat_old,
  ...
  for (g in 1:length(num_items_per_group)) {
    ...
    # root node location of group g
    root_location_g <- root_location[indices]
...
ddt_lcm_mcmc <- function(K, data, num_items_per_group,
  ...
  dist_mat_old = NULL
  root_location <- unlist(tree_phylo4d@data[as.character(rootNode(tree_phylo4d)), item_labels])
  leaf_data <- as.matrix(tree_phylo4d@data[names(tipLabels(tree_phylo4d)), item_labels])
  rownames(leaf_data) <- tipLabels(tree_phylo4d)
  locations_root_diff <- t(t(leaf_data) - root_location)
  leaf_node_labels <- paste0("v", 1:K)

  # controls
  fix_tree <- controls$fix_tree
  is_linear <- controls$is_linear

  if (is_linear){
    sample_c <- sample_c_linear
  } else{
    sample_c <- sample_c_quadratic
  }
  ...
  selection_probs <- matrix(0, nrow = K, ncol = length(num_items_per_group))
  ...
  logllk_model <- rep(0, total_iters+1)
  logllk_model_lcm <- rep(0, total_iters+1)

  # get the information of the initial tree
  init_info <- logllk_ddt(c, is_linear, Sigma_by_group, tree_phylo4d, num_items_per_group,
                          tree_structure_old = NULL, dist_mat_old = NULL)
  ...
  for(iter in 1:total_iters+1){
    ...
    if (!fix_tree){
      sampled_tree <- sample_tree_topology(tree_phylo4d_old, Sigma_by_group, num_items_per_group, c = c, is_linear,
                                           tree_structure_old = tree_structure_old, dist_mat_old = dist_mat_old)#NULL
      accept[iter-1] <- sampled_tree$accept
      ...
      if (sampled_tree$accept) {
        ...
        root_location <- unlist(tree_phylo4d_old@data[as.character(rootNode(tree_phylo4d_old)), item_labels])
    ...
    # cat("sampled tree \n")
    # restart every xx iterations
    # if (iter %% 500 == 1){
    #   selected_iteration <- sample(1:iter, 1)
    #   tree_phylo4d_old <- tree_list[[selected_iteration]]
    #   root_location <- unlist(tree_phylo4d_old@data[as.character(rootNode(tree_phylo4d_old)), 1:J])
    # }
    ...
    Sigma_by_group_samples[,iter-1] <- Sigma_by_group
    ...
    # if (iter %% 100 == 0){
    sampled <- sample_leaf_locations_pg(num_items_per_group, root_location, dist_mat_old,
                             Sigma_by_group, pg_mat, a_pg, auxiliary_mat, auxiliary_mat_range, class_assignments)
    ...
    response_probs_samples[,iter-1] <- expit(leaf_data)
    class_probs_samples[,iter-1] <- class_probability
    Z_samples[, iter-1] <- class_assignments
  ...
  result <- list(
    ...
    selection_probs = selection_probs / total_iters,
