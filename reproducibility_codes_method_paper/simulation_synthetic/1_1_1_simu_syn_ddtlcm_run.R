...
#' fix_tree_at options:
#' "FALSE": will be estimated; "truth": fix at true tree; "misspecified": misspecified tree
fix_tree_at = as.character(args[4]) 
...
c_init <- rgamma(1, 5, 1)
# initialize randomly
initialization <- initialize(K, response_matrix, num_items_per_group = num_items_per_group, c = c_init,
...
if (fix_tree_at == "truth"){
  ...
  if (tree_idx == 3){
    tr_txt <- "(((v1:0.85, v2:0.85):0.1, v3:0.95):0.05);"
    tree_wrong <- read.tree(text = tr_txt)
    tree_wrong$node.label <- paste0("u", 1:Nnode(tree_wrong))
  } else if (tree_idx == 4){
    tr_txt <- "(((v1:0.85, v2:0.85):0.1, v3:0.95):0.05);"
    tree_wrong <- read.tree(text = tr_txt)
    tree_wrong$node.label <- paste0("u", 1:Nnode(tree_wrong))
    # plot(tree_wrong, show.node.label = T)
  } else if (tree_idx == 1){
    tr_txt <- "(((v1:0.25, v2:0.25):0.15, v3:0.4):0.6);"
    tree_wrong <- read.tree(text = tr_txt)
    tree_wrong$node.label <- paste0("u", 1:Nnode(tree_wrong))
  ...
  tree_wrong <- phylo4d(tree_wrong)
  tree_wrong@data <- initial_tree@data
  initials$tree_phylo4d <- tree_wrong
}
controls <- list(
  fix_tree = fix_tree,
  is_linear = is_linear
...
burnin <- 50
system.time({
  result <- ddt_lcm_mcmc(K, data = response_matrix, 
...
dat_result[row_idx, c("label", "N")] <- c("DDT-LCM", N)
...
summarized_result_ddtlcm <- dat_result
...
save_dir <- paste0(save_dir, "/ddtlcm_fixtree_", fix_tree_at)
if (!dir.exists(save_dir)){
  print("create")
  dir.create(save_dir)
}
...
save(sim_data, seed_theta, seed_Y, result, 
     tree_idx, summarized_result_ddtlcm, Sigma_dat, file = file_name)
